  ...
  useEffect(()=>{
    setTimeout(()=>{ 
      if(user?.name !== undefined) {
        toast(`Bem vindo ${user?.name}!`, {icon: '👋', style: { background: '#f0ecb9' }, duration: 2000});
      } 
     }, 1500);
  }, [user])
    ...
    <div id="page-room">
      ...
      <main className="content">
        <div className="room-title">
          ...
          {questions.length > 0 ? <span>{questions.length} pergunta(s)</span> : <span style={{ background: '#835AFD' }}>Nenhuma pergunta ainda!</span>}
        </div>
        ...
        <form onSubmit={handleSendQuestion}>
          ...
          <div className="form-footer">
            { user ? (
              ...
              <span>Para enviar uma pergunta e dar likes, <button onClick={handleLoginGoogle}>faça seu login.</button></span>
            )}
